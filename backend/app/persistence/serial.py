...
def to_json(state: GameState) -> str:
    payload: dict[str, object] = {
        ...
        "quiz_conceitual": (
            ...
            else {"senso": int(state.quiz_conceitual.senso), "respondido": state.quiz_conceitual.respondido}
        ),
...
def from_json(raw: str) -> GameState:
    ...
    quiz_raw = d.get("quiz_conceitual")
    quiz_conceitual = None
    if isinstance(quiz_raw, dict):
        from app.domain.sensos import Senso as _Senso
        quiz_conceitual = QuizConceitual(
            senso=_Senso(_int(quiz_raw, "senso")),
            respondido=_bool(quiz_raw, "respondido"),
        )
    badges_raw = d["badges"]
    ...
    return GameState(
        ...
        desafio=desafio,
        quiz_conceitual=quiz_conceitual,
