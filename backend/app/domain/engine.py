...
from . import content, quiz, scoring, situacoes
from .decay import (
    DURACAO_DESAFIO,
    FATOR_CHOQUE,
    INTERVALO_CHOQUE,
...
def apply(state: GameState, ctype: str, payload: dict[str, object], now: float) -> CommandOutcome:
    ...
    if ctype == "quiz.responder":
        return _quiz_responder(state, payload, now)
    if ctype == "fase.avancar":
        return _avancar(state, now)
    raise UnknownCommand(ctype)


_SEIRI_EXPLICACAO: dict[SeiriZona, str] = {
    SeiriZona.MANTER: (
        "✅ Manter — itens de uso frequente e necessário ficam no posto."
    ),
    SeiriZona.RED_TAG: (
        "🏷️ Etiqueta Vermelha — itens de uso raro, duvidoso ou de outro setor "
        "são etiquetados para avaliação antes de descartar."
    ),
    SeiriZona.DESCARTAR: (
        "🗑️ Descartar — itens danificados, vencidos ou sem nenhuma serventia "
        "saem imediatamente do posto."
    ),
}
...
def _seiri(state: GameState, payload: dict[str, object]) -> CommandOutcome:
    ...
    if correto:
        # Só marca como resolvido quando acerta — erros devolvem o item à fila.
        item.resolvido = zona
        _set_radar(state, Senso.SEIRI, sum(i.resolvido == i.destino for i in state.seiri), len(state.seiri))
        state.score += scoring.pontos_seiri_acerto(item.erros, state.streak)
        _registrar(state, True)
        _talvez_desafio(state)
        sufixo = f" (−{item.erros * scoring.PENALIDADE_ERRO_SEIRI} pts por erros anteriores)" if item.erros > 0 else ""
        return CommandOutcome(True, "comemora", f"Isso! {item.nome} no lugar certo.{sufixo}")

    # Errou: incrementa contador, não pontua, item volta para a fila.
    item.erros += 1
    _registrar(state, False)
    explicacao = _SEIRI_EXPLICACAO[item.destino]
    msg = f"Não é bem assim. {explicacao} Tente de novo!"
    return CommandOutcome(False, "pergunta", msg)
...
def _quiz_responder(state: GameState, payload: dict[str, object], now: float) -> CommandOutcome:
    """Valida a resposta do quiz conceitual e, se correto ou não, avança a fase."""
    if state.quiz_conceitual is None or state.quiz_conceitual.respondido:
        raise UnknownCommand("nenhum quiz conceitual ativo")
    opcao = _i(payload, "opcao")
    try:
        acertou, pontos = quiz.responder(state.quiz_conceitual.senso, opcao)
    except ValueError as exc:
        raise UnknownCommand(str(exc)) from exc

    state.score += pontos
    state.quiz_conceitual.respondido = True

    # Avança a fase agora que o quiz foi respondido.
    outcome = _avancar_fase(state, now)

    if acertou:
        msg = f"Resposta certa! +{pontos} pts. {outcome.mensagem}"
        return CommandOutcome(True, "comemora", msg)
    msg = f"Não era essa, mas vamos em frente. {outcome.mensagem}"
    return CommandOutcome(False, "boasvindas", msg)


def _avancar(state: GameState, now: float) -> CommandOutcome:
    """Tenta avançar a fase. Abre o quiz de gate se ainda não respondido."""
    if state.current_phase == Senso.SHITSUKE:
        _processar_shitsuke(state, now)
        if state.shitsuke_sustentado:
            # Quiz de gate para Shitsuke antes de finalizar.
            if state.quiz_conceitual is None:
                state.quiz_conceitual = QuizConceitual(senso=Senso.SHITSUKE)
                return CommandOutcome(
                    None, "pergunta", "Quase lá! Responda a pergunta final antes de concluir."
                )
            if not state.quiz_conceitual.respondido:
                return CommandOutcome(None, "pergunta", "Responda o quiz conceitual para concluir.")
            state.finished = True
    ...
    # Abre quiz de gate se ainda não foi criado/respondido.
    if state.quiz_conceitual is None or state.quiz_conceitual.senso != state.current_phase:
        state.quiz_conceitual = QuizConceitual(senso=state.current_phase)
        return CommandOutcome(
            None, "pergunta", "Fase dominada! Responda o quiz antes de avançar."
        )
    if not state.quiz_conceitual.respondido:
        return CommandOutcome(None, "pergunta", "Responda o quiz conceitual para avançar.")
    return _avancar_fase(state, now)


def _avancar_fase(state: GameState, now: float) -> CommandOutcome:
    """Efetiva o avanço de fase (chamado após quiz respondido)."""
    ...
    state.last_decay_at = now
    state.quiz_conceitual = None  # limpa para a próxima fase
    if state.current_phase == Senso.SHITSUKE:
        state.shitsuke_last_shock_at = now
