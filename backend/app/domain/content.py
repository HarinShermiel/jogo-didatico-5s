...
_SEIRI_POOL: tuple[tuple[str, str, str, SeiriZona], ...] = (
    ("Chave de fenda em uso diário", "🪛", "Ferramenta do dia a dia do posto", SeiriZona.MANTER),
    ("Paquímetro do operador", "📏", "Instrumento de medição do operador", SeiriZona.MANTER),
    ("EPI (luva) em bom estado", "🧤", "Equipamento de proteção individual", SeiriZona.MANTER),
    ("Manual do equipamento", "📘", "Documentação técnica da máquina", SeiriZona.MANTER),
    ("Torquímetro caro de uso raro", "🔧", "Ferramenta de precisão, usada ocasionalmente", SeiriZona.RED_TAG),
    ("Molde de produto descontinuado", "🧱", "Produto parado há meses, pode ter uso futuro", SeiriZona.RED_TAG),
    ("Peça boa de outro setor", "⚙️", "Peça em bom estado, mas não pertence a este posto", SeiriZona.RED_TAG),
    ("Refugo / peça quebrada", "🗑️", "Peça danificada sem possibilidade de reparo", SeiriZona.DESCARTAR),
    ("EPI vencido", "🦺", "Equipamento com data de validade expirada", SeiriZona.DESCARTAR),
    ("Embalagem vazia amassada", "📦", "Caixa sem uso acumulada no posto", SeiriZona.DESCARTAR),
    ("Estopa usada com óleo", "🧽", "Material absorvente já utilizado", SeiriZona.DESCARTAR),
    ("Duplicata de ferramenta sem uso", "🔨", "Ferramenta idêntica a outra já no posto", SeiriZona.RED_TAG),
)
