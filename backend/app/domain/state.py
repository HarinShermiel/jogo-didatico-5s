...
class SeiriItem:
    ...
    erros: int = 0  # tentativas erradas antes de acertar
...
@dataclass
class QuizConceitual:
    """Quiz de gate entre fases: pergunta conceitual sobre o senso concluído."""
...
class GameState:
    ...
    quiz_conceitual: QuizConceitual | None = None
...
def public_view(state: GameState) -> dict[str, object]:
    ...
    return {
        ...
        "quizConceitual": _public_quiz(state),
        "shitsukeDesafio": {
...
def _public_quiz(state: GameState) -> dict[str, object] | None:
    """Projeção pública do quiz conceitual — sem gabarito."""
    if state.quiz_conceitual is None:
        return None
    if state.quiz_conceitual.respondido:
        return None
    return quiz.public_quiz(state.quiz_conceitual.senso)
