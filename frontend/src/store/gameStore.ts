...
export const useGameStore = create<GameStore>((set, get) => ({
  token: null,
  state: null,
  mentor: { mood: 'boasvindas', mensagem: BOAS_VINDAS, correto: null },
  loading: false,
  error: null,
  onboarding: true,
  ...
  dispatch: async (type, payload = {}) => {
    ...
    try {
      ...
      set({
        state: resp.state,
        mentor: { mood: resp.feedback.mentor, mensagem: resp.feedback.mensagem, correto: resp.feedback.correto },
        error: null,
      })
    } catch (e) {
      ...
      if (err.status === 422) {
        set({
          mentor: { mood: 'pergunta', mensagem: 'Calma! Esse ritmo não parece humano. 😅', correto: null },
          error: null,
        })
      } else {
  ...
  setMentor: (mood, mensagem) => set({ mentor: { mood, mensagem, correto: null } }),
