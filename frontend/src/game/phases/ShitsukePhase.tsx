...
export function ShitsukePhase({ itens }: Props): JSX.Element {
  ...
  return (
    ...
    <div className="mx-auto flex min-h-screen max-w-3xl flex-col justify-center gap-4 p-4 pt-24">
      <motion.div
        animate={{ borderColor: sustentado ? '#3FA34D' : acimaMeta ? '#C9A227' : '#E4572E' }}
        className="flex items-center gap-4 rounded-3xl border-2 bg-white/10 p-4 text-white shadow-xl"
      >
      ...
      <div className="rounded-3xl bg-white/10 p-4 shadow-xl">
        <p className="mb-3 text-sm font-bold text-white/85">
          🔍 Checklist de auditoria 5S — corrija para reerguer o radar
        </p>
          ...
          <AnimatePresence>
            {itens.map((item) => (
              <motion.li
                ...
                style={{ boxShadow: `inset 5px 0 0 ${SENSO_COR[item.senso]}` }}
              >
                ...
                <motion.button
                  ...
                  className="shrink-0 rounded-xl bg-marca-azul px-3 py-1.5 text-xs font-bold text-white hover:brightness-125"
